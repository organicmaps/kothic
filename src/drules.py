...
def serialize_text(containers, variant_names):
    ...
    for idx, entry in enumerate(palette):
        out.append("  " + _color_id(idx) + " " + " ".join(f"#{v:08X}" for v in entry))
    if named:
        out.append("named-colors:")
        for name, color_idx in named:
            out.append(f"  {name} {_color_id(color_idx)}")

    cap = {ROUNDCAP: "round", BUTTCAP: "butt", SQUARECAP: "square"}
    join = {ROUNDJOIN: "round", BEVELJOIN: "bevel", NOJOIN: "no"}

    def is_transparent(color_idx):  # a halo/outline color that is zero in every variant == "none"
        return all(v == 0 for v in palette[color_idx])

    def line_str(ln, with_priority):
        parts = [f"width={_g(ln.width)}", f"color={_color_id(ln.color)}",
    ...
    def captiondef_str(cd):
        parts = [f"height={cd.height}", f"color={_color_id(cd.color)}"]
        if not is_transparent(cd.stroke_color):
            parts.append(f"stroke={_color_id(cd.stroke_color)}")
        if cd.offset_x:
    ...
    for ce in template.cont:
        ...
        for el in ce.element:
            ...
            if el.area._is_set():
                parts = [f"color={_color_id(el.area.color)}"]
                if el.area.border._is_set():
            ...
            if el.shield._is_set():
                sh = el.shield
                parts = [f"height={sh.height}", f"color={_color_id(sh.color)}"]
                if not is_transparent(sh.stroke_color):
                    parts.append(f"stroke={_color_id(sh.stroke_color)}")
                parts.append(f"text_color={_color_id(sh.text_color)}")
                if not is_transparent(sh.text_stroke_color):
                    parts.append(f"text_stroke={_color_id(sh.text_stroke_color)}")
